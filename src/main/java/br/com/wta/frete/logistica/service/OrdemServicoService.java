...
public class OrdemServicoService {
        ...
        public OrdemServicoResponse criarOrdemServico(OrdemServicoRequest request) {
                ...
                DetalheCliente cliente = detalheClienteRepository.findById(request.clienteSolicitanteId())
                                .orElseThrow(() -> new ResourceNotFoundException(
                                                "Cliente Solicitante não encontrado com ID: "
                ...
                if (request.transportadorDesignadoId() != null) {
                        transportador = transportadorRepository.findById(request.transportadorDesignadoId())
                                        .orElseThrow(() -> new ResourceNotFoundException(
                                                        "Transportador Designado não encontrado com ID: "
                                                                        + request.transportadorDesignadoId()));
                }
        ...
        /**
         * Documentação: Confirma a Ordem de Serviço (OS) com os dados do lance
         * vencedor.
         * <p>
         * Fluxo: Utilizado pelo FreteService (@Scheduled) após o leilão ser finalizado.
         * Atualiza o Transportador Designado, o valor final negociado e o Status da OS
         * para CONFIRMADO.
         * </p>
         *
         * @param ordemServicoId O ID da Ordem de Serviço a ser atualizada.
         * @param lanceVencedor  A entidade Lance que venceu o leilão.
         * @return O DTO de resposta da Ordem de Serviço atualizada.
         * @throws ResourceNotFoundException Se a OS ou Transportador não forem
         *                                   encontrados.
         */
        @SuppressWarnings("null")
        @Transactional
        public OrdemServicoResponse confirmarOsComLanceVencedor(Long ordemServicoId, Lance lanceVencedor) {

                // 1. BUSCA DA OS EXISTENTE
                OrdemServico ordem = ordemServicoRepository.findById(ordemServicoId)
                                .orElseThrow(() -> new ResourceNotFoundException(
                                                "Ordem de Serviço não encontrada com ID: " + ordemServicoId));

                // 2. BUSCA DO TRANSPORTADOR VENCEDOR (Garante que a entidade está carregada)
                // O ID do transportador vem do Lance
                Long transportadorId = lanceVencedor.getTransportador().getPessoaId();
                Transportador transportadorVencedor = transportadorRepository.findById(transportadorId)
                                .orElseThrow(() -> new ResourceNotFoundException(
                                                "Transportador Vencedor não encontrado com ID: " + transportadorId));

                // 3. ATUALIZAÇÃO DOS CAMPOS
                // O valor final do frete é o valor do lance vencedor.
                // NOTA: Assumindo que a entidade OrdemServico possui o campo
                // setValorFinalNegociado
                // ordem.setValorFinalNegociado(lanceVencedor.getValorLance());

                // Transportador designado
                ordem.setTransportadorDesignado(transportadorVencedor);

                // Status: De PENDENTE (pré-leilão) para CONFIRMADO (pós-leilão)
                ordem.setStatus(StatusServico.CONFIRMADO);
